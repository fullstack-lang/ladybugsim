...
func fillUpTree(
	...
	for _, gongStruct := range sliceOfGongStructsSorted {
		...
		case "Ladybug":
			...
			set := *models.GetGongstructInstancesSet[models.Ladybug](playground.stageOfInterest)
			for _ladybug := range set {
				nodeInstance := (&tree.Node{Name: _ladybug.GetName()}).Stage(playground.treeStage)
				nodeInstance.IsNodeClickable = true
				nodeInstance.Impl = NewInstanceNodeCallback(_ladybug, "Ladybug", playground)

				nodeGongstruct.Children = append(nodeGongstruct.Children, nodeInstance)
			}
		case "LadybugSimulation":
			nodeGongstruct.Name = name
			set := *models.GetGongstructInstancesSet[models.LadybugSimulation](playground.stageOfInterest)
			for _ladybugsimulation := range set {
				nodeInstance := (&tree.Node{Name: _ladybugsimulation.GetName()}).Stage(playground.treeStage)
				nodeInstance.IsNodeClickable = true
				nodeInstance.Impl = NewInstanceNodeCallback(_ladybugsimulation, "LadybugSimulation", playground)
